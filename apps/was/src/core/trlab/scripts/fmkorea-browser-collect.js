...
import { extractAnchors } from '#trlab/modules/services/signals/collector-utils';

const BASE_URL = process.env.TRLAB_URL ?? process.env.WAS_URL ?? 'http://localhost:5174';
...
const AUTH_WAIT_MS = Number(process.env.FMKOREA_AUTH_WAIT_MS ?? 45000);
...
async function collect() {
  ...
  clearChromeProfileLocks(PROFILE_DIR);
  ...
  try {
    ...
    const payload = { startedAt, finishedAt: new Date().toISOString(), items, reason: AUTH_MODE ? 'fmkorea-browser-auth' : 'fmkorea-browser-worker' };
    if (NO_IMPORT) {
      console.log(JSON.stringify({ ok: true, source: 'FMKorea', count: items.length, ...payload }));
      return;
    }
    await importSignals(payload);
...
async function waitForUser(page) {
  ...
  if (process.stdin.isTTY) {
    ...
    await page.waitForTimeout(AUTH_WAIT_MS);
  }
...
function clearChromeProfileLocks(profileDir) {
  for (const relativePath of ['SingletonCookie', 'SingletonLock', 'SingletonSocket', 'RunningChromeVersion', path.join('Default', 'LOCK')]) {
    fs.rmSync(path.join(profileDir, relativePath), { force: true, recursive: true });
  }
}
