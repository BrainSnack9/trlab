...
export function compareCandidates(a, b) {
  return (b.production?.score ?? 0) - (a.production?.score ?? 0)
    || (b.scoring?.opportunity ?? 0) - (a.scoring?.opportunity ?? 0)
    || (a.scoring?.cohesionPenalty ?? 0) - (b.scoring?.cohesionPenalty ?? 0)
...
function getBusinessPriority(candidate) {
  ...
  score += Math.min(20, candidate.scoring?.opportunity ?? 0);
  score -= Math.min(24, candidate.scoring?.cohesionPenalty ?? 0);
