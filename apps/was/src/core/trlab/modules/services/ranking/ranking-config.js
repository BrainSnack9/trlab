...
export const interestAreas = [
  ...
  ['travel', '여행', ['여행', '항공', '호텔', '일본', '마카오', '축제', '관광']],
  ['parenting', '육아', ['육아', '아이', '아기', '엄마', '아빠', '키즈', '어린이집', '부모급여']],
  ['education', '교육', ['교육', '학교', '입시', '수능', '강의']],
  ['entertainment', '엔터', ['배우', '드라마', '영화', '아이돌', '방송', '할리우드']],
...
export const politicsPattern = /(대통령|민주당|국민의힘|선거|국회|정당|후보|투표|사전\s*투표|개표|선관위|교육감|대선|지선|부정선거|이재명|조국|한동훈)/i;
...
export const contentOpportunityPattern = /(GEO|AEO|SEO|AI\s*검색|검색\s*노출|브랜드\s*노출|생성형\s*AI|업무\s*자동화|AX|GLP-?1|위고비|마운자로|비만치료제|유지어터|건강기능식품|건기식|수면\s*영양제|멜라토닌|반려동물|펫|강아지|고양이|병원비|진료비|표준수가|보험|AI\s*장난감|AI\s*펫|펫가젯|소비자물가|물가\s*품목|밀키트|1인\s*가구|전기차|구독|배터리|K-?뷰티|성분\s*중심|올리브영|단백질\s*시장|여행\s*소비|지원금|반값여행|체크리스트|비교|주의|전략|변화|시장\s*확대)/i;
...
export const canonicalKeywordMap = new Map([
  ...
  ['네이버페이', '네이버페이'],
  ['glp-1', 'GLP-1'], ['glp1', 'GLP-1'], ['geo', 'GEO'], ['aeo', 'AEO'], ['seo', 'SEO']
]);
export const areaOverrideMap = new Map([
  ['치지직', 'tech'], ['스트로베리', 'tech'], ['anthropic', 'tech'],
  ['AI 검색', 'tech'], ['생성형 AI', 'tech'], ['AI 반도체', 'tech'],
  ['AI 검색 노출 전략', 'tech'], ['AI 장난감', 'tech'], ['AI 펫가젯', 'tech'],
  ['삼성전자', 'brand'], ['하이닉스', 'brand'], ['msi', 'game'], ['페이즈', 'game'],
  ['브래드 피트', 'entertainment'], ['제니퍼 애니스톤', 'entertainment'], ['안젤리나 졸리', 'entertainment'],
  ...
  ['출산', 'parenting'], ['육아', 'parenting'], ['어린이집', 'parenting'], ['부모급여', 'parenting'],
  ['반려동물', 'pet'], ['강아지', 'pet'], ['고양이', 'pet'], ['펫', 'pet'],
  ['반려동물 진료비 표준수가제', 'pet'], ['GLP-1 이후 유지어터 시장', 'health'],
  ['소비자물가 품목 변화', 'economy'], ['일상 건강 단백질 시장', 'health']
]);
...
export const noiseKeywords = new Set('뉴스 속보 단독 조회수 게시글 커뮤니티 갤러리 포텐 유튜브 인스타 네이버 다음 nate osen co kr 공식 요약 정리 리뷰 추천 인기 메뉴 로그인 회원가입 연합뉴스 조선일보 중앙일보 동아일보 매일경제 재팬코리아데일리 한경매거진 매거진'.split(' '));
