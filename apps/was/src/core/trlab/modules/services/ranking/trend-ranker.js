...
import { classifyTitle, extractKeywordTokens, extractNgrams, isUsefulCandidate, normalizeCandidateKeyword } from './ranking-text.js';
...
function finalizeCandidate(candidate) {
  ...
  return {
    ...
    evidence: uniqueSignals(candidate.signals).slice(0, 5).map((signal) => ({ source: signal.source, title: signal.title, url: signal.url, metric: signal.metric, seenCount: signal.seenCount }))
  };
}
