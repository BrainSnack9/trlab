...
import { ClipboardList, Copy, Download, Images, Palette, Search } from 'lucide-react';
...
export function CardNewsMaker({ studio, plan }) {
  ...
  }, [draftKey, loadedDraftKey, safeSelected, resolvedStyleKey, channelName, generatedImages, generatedImageHistory, cards]);

  function setCardGeneratedImage(card, index, image) {
    const key = cardImageKey(card, index);
    setGeneratedImages((value) => ({ ...value, [key]: image }));
    setGeneratedImageHistory((value) => ({
      ...value,
      [key]: addImageToHistory(value[key], image)
    }));
  }
  ...
  return (
    <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_420px]">
      <CardWorkspace
        ...
        generatedImageHistory={generatedImageHistory[cardImageKey(card, safeSelected)] ?? []}
        setGeneratedImage={(image) => setCardGeneratedImage(card, safeSelected, image)}
        selectGeneratedImage={(image) => setGeneratedImages((value) => ({ ...value, [cardImageKey(card, safeSelected)]: image }))}
      />
...
function CardWorkspace({ cards, selected, setSelected, card, style, studio, plan, generatedImage, generatedImageHistory, setGeneratedImage, selectGeneratedImage }) {
  return (
  ...
  return (
    <Card className="h-fit">
      ...
      <CardContent className="space-y-4">
        ...
        <SelectedProductionBrief card={card} />
...
function SelectedProductionBrief({ card }) {
  ...
  return (
    <div className="rounded-lg border bg-white p-3">
      <div className="mb-2 flex items-center justify-between gap-2">
        <div className="flex items-center gap-2 text-sm font-black"><ClipboardList className="h-4 w-4 text-indigo-600" />선택 카드 제작 지시</div>
      ...
      {brief.pexelsQuery ? (
        <div className="mt-2 flex items-center gap-2 rounded-md border border-indigo-100 bg-indigo-50 px-2 py-1.5 text-xs font-black text-indigo-700">
          <Search className="h-3.5 w-3.5" />
          <span className="min-w-0 flex-1 break-words">{brief.pexelsQuery}</span>
          <Button size="sm" variant="outline" onClick={() => navigator.clipboard?.writeText(brief.pexelsQuery)}>복사</Button>
        </div>
      ) : null}
      {candidates.length ? (
...
function sanitizeGeneratedImageHistory(value, currentImages = {}, cards = []) {
  const allowed = new Set(cards.map((card, index) => cardImageKey(card, index)));
  ...
  for (const key of allowed) {
    ...
    const history = current?.url ? addImageToHistory(items, current) : items;
    const cleaned = history.filter((image) => image && typeof image === 'object' && typeof image.url === 'string');
    if (cleaned.length) result[key] = dedupeImages(cleaned).slice(0, 12);
  }
  return result;
}

function addImageToHistory(history = [], image) {
  if (!image?.url) return Array.isArray(history) ? history : [];
  return dedupeImages([image, ...(Array.isArray(history) ? history : [])]).slice(0, 12);
}
