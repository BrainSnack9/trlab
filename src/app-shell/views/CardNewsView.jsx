...
function useContentPlan(studio, contentPlans, setContentPlans, refreshToken) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/content/plan${refreshToken ? '?refresh=1' : ''}`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify(studio) })
      .then((response) => response.ok ? response.json() : Promise.reject(new Error(`시나리오 생성 실패: ${response.status}`)))
