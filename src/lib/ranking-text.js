import {
  ...
  genericKeywords, interestAreas, noiseKeywords, stopWords
} from './ranking-config';
...
function isUsefulSinglePart(value) {
  ...
  return !/(에서|에게|으로|로서|처럼|인데|면서|던데|된다|하다|세요|있음|하고|가지고|개|시장|전망)$/.test(value);
}

export function isWeakPhraseToken(value) {
  return /(가|이|을|를|로|으로|에서|에게|부터|까지|하고|된다|하다|하는|하고|면서|던데|개|시장|전망)$/.test(value);
}
