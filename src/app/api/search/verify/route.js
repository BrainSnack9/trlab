import { verifySearch } from '@/lib/ranking-search';
...
export async function GET(request) {
  ...
  const type = params.get('type')?.trim() ?? '검증형';
  ...
  return Response.json({
    ...
    verification: {
      ...
      reason: `검색결과 매칭 ${result.matchedResults}건 · ${sources.map((source) => `${source.source} ${source.status}`).join(', ')}`,
